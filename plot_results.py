...
def plot_main(ax, k, data, bf, bf_smooth, bf_bao, ylabel=None):
    ...
    ax.plot(k, bf / bf_smooth, color='blue', lw=4, label="PF")
    ax.plot(k, bf_bao / bf_smooth, color='red', lw=4, label="BAO")
    ax.set_xlim((k[0], k[-1]))
    ax.set_ylim((0.8, 1.2))
    ax.grid()
    ax.legend()
...
def plot_results(mcmc, likelihood, theory, DATA, COV_NGC, COV_SGC, k, FIG_PATH, handle):
    """
